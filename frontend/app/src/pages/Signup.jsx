...
function FormStep({ T, onSubmit, isSubmitting, error, onGoogle }) {
...
export default function Signup() {
  const { T } = useTheme();
  const { register: registerUser, verifyOtp, isAuthenticated, error, clearError } = useAuth();
  const navigate = useNavigate();

  const [step, setStep] = useState(() => sessionStorage.getItem(SESSION_STEP) || 'form');
  const [email, setEmail] = useState(() => sessionStorage.getItem(SESSION_EMAIL) || '');
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        {step === 'verify'
          ...
          : <FormStep T={T} onSubmit={onRegister} isSubmitting={isSubmitting} error={error} onGoogle={onGoogle} />
        }
      </div>
    </div>
  );
